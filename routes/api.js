...
router.post('/getquote', async (req, res) => {
  ...
  twig.renderFile(
    ...
    { message: 'team/quote', quote },
    (e, h) => {
      email.sendEmail(
        'support@frequentresearch.com',
        'Quote Request From T2O Website',
        h,
        h,
      )
  ...
  res.json({
    ...
    message: {
      ...
      body:
        'Your request has been sent to our team. One of our team members will get in touch with you shortly.',
    },
  })
})
...
router.get('/profileoptions', async (req, res) => {
  data = await ProfileOptions.find({
    name: {
      $in: [
        'empstat',
        'industry',
...
router.get('/exrate/:cur1/:cur2', (req, res) => {
  ...
  res.json({
    rate: exrate.rates[req.params.cur2] / exrate.rates[req.params.cur1],
  })
})
router.get('/getCurrency', (req, res) => {
  currencies = JSON.parse(
    require('fs').readFileSync('./databank/theworld.json'),
  )
...
router.get('/earnings', async (req, res) => {
  ...
  await redeem.forEach(async (e) => {
    amount = await e.total.reduce(
      (a, b) => a + Number(b.toString().replace(/[^\d.-]/g, '')),
      0,
    )
    min =
      min == null || e._id.year * 100 + e._id.month < min
        ? e._id.year * 100 + e._id.month
        : min
    max =
      max < e._id.year * 100 + e._id.month
        ? e._id.year * 100 + e._id.month
        : max
  ...
  data.total = await data.amount.reduce(
    (e) => e + Number(e.toString().replace(/[^\d.-]/g, '')),
    0,
  )
  res.json(data)
})
...
router.get('/professions/industry', async (req, res) => {
  professions = JSON.parse(
    require('fs').readFileSync('./databank/professions.json'),
  )
  let industry = []
...
router.get('/professions/:industry', async (req, res) => {
  professions = JSON.parse(
    require('fs').readFileSync('./databank/professions.json'),
  ).filter((e) => e.Industry === req.params.industry)
  let department = []
  await professions.forEach((e) => {
...
router.get('/professions/:industry/:department', async (req, res) => {
  professions = JSON.parse(
    ...
    .filter(
      (e) =>
        e.Industry === req.params.industry &&
        e.Department === req.params.department,
    )
    .map((e) => e.Jobrole)
...
router.post('/zone', async (req, res) => {
  ...
  let zip = await ProfileOptions.findOne({
    _id: mongoose.Types.ObjectId(req.body.country),
  })
  ProfileOptions.find({ dependency: mongoose.Types.ObjectId(dependency[0]) })
    .then(async (data) => {
...
router.post('/save/:field', async (req, res) => {
  ...
  if (mongoose.isValidObjectId(user[req.params.field]))
    options = await ProfileOptions.find({
      dependency: mongoose.Types.ObjectId(user[req.params.field]),
    })
...
router.post('/zipcode', async (req, res) => {
  ...
  options = await ProfileOptions.find({
    name: 'country',
    label: countries[req.body.cn].name,
  })
...
router.route('/automobile/:type').get((req, res) => {
  let data = JSON.parse(
    require('fs').readFileSync('./databank/automobile.json'),
  )
...
router
  ...
  .get(async (req, res) => {
    try {
      let opts=Array.from(await ProfileOptions.find({ name: req.params.component }));
      console.log({line403:opts,name: req.params.component});
      await opts.map((e) => {
  ...
  .post(async (req, res) => {
    ...
    let data = Array.from(
      await ProfileOptions.find({ name: req.params.component }),
    ).filter((info) => {
      out = false
      for (let i = 0; i < dependency.length; i++) {
        out = out || info.dependency.indexOf(dependency[i]) !== -1
...
router
  ...
  .post(async (req, res) => {
    ...
    ProfileOptions.find({ name: req.params.component })
      .then((data) => {
