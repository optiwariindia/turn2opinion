callAPI = async function (url, method, info) {
  ...
  if (method.toUpperCase() !== 'GET') options.body = JSON.stringify(info)
  ...
  add: function (e) {
    ...
    if (vals.indexOf(e.innerText) === -1) {
      ...
      p.querySelector('.input').innerHTML =
        p.querySelector('.input').innerHTML +
        `<span><i onclick=inputCheckbox.remove(this) class="fa fa-times"></i> ${e.innerText}</span>`
  ...
  init: function (e) {
    ...
    if (inpgrp.querySelector('.input').innerText !== vals) {
      for (let index = 0; index < val.length; index++) {
        ...
        inpgrp.querySelector('.input').innerHTML =
          inpgrp.querySelector('.input').innerHTML +
          `<span><i onclick=inputCheckbox.remove(this) class="fa fa-times"></i> ${element}</span>`
      }
    }
  },
}
...
const progress = {
  ...
  show: async function () {
    ...
    await document.querySelectorAll('[page]').forEach((e) => {
      ...
      if (e.getAttribute('page') == progress.value) e.style.display = 'block'
    })
...
showFields = {
  ...
  city: () => {
    ...
    callAPI('/api/v1/country', 'get', {}).then((data) => {
      ...
      if (Object.keys(data).indexOf('name') == -1) info = data[cn].states[st]
      else info = data.states[st]
      options = `<option value="" disabled selected>Select</option>`
  ...
  localize: function (mystring) {
    ...
    for (i = 0; i < mystring.length; i++) {
      ...
      if (data[i] != null) {
        ...
        for (j = 0; j < extradigit; j++) output += unitdigit
      }
    }
    return output
  },
